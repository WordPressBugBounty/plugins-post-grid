...
class PostGridRest
{
	...
	public function register_routes()
	{
		...
		register_rest_route(
			...
			array(
				'methods' => 'POST',
				'callback' => array($this, 'post_grid_data'),
				'permission_callback' => function () {
					return current_user_can('manage_options');
				},
	 ...
	 * @param WP_REST_Request $post_data Post data.
	 */
	public function post_grid_data($post_data)
	{
	...
	public function post_list($post_data)
	{
		...
		$prevText = !empty($post_data['prevText']) ? $post_data['prevText'] : __("Previous", "post-grid");
		$nextText = !empty($post_data['nextText']) ? $post_data['nextText'] : __("Next", "post-grid");
		$maxPageNum = !empty($post_data['maxPageNum']) ? $post_data['maxPageNum'] : 0;
